...
export class AuthService {
  ...
  public register(myForm: NgForm) {
    const { fullName, email, phone, branch, password } = myForm.value;
    const data = {
      fullName,
      phone,
      branch,
    ...
    this.http
      .post(`${this.basic_url}/register`, data, { responseType: 'text' })
      .subscribe(
        (result) => {
        ...
        (errors) => {
          this.toast.error({ detail: errors.error, duration: 2000 });
        },
        () => {}
      );
  }
  ...
  public login(myForm: NgForm, authentication_type: string) {
    ...
    this.http
      .post<LoginResponce>(`${this.basic_url}/login`, data, {
        headers: {
  ...
  editProfile(myForm: any, id: any) {
    ...
    console.log(data);
    this.http
      .put(`${this.basic_url}/editProfile`, data, { responseType: 'text' })
      .subscribe(
        (result) => {
          this.userEroor = true;
  ...
  getMyProduitNonValide(email: any): Observable<Object[]> {
    return this.http.get<Object[]>(
      this.basic_url + '/NonValidProductUser/' + email
    );
  }
  getMyProduitValide(email: any): Observable<Object[]> {
    return this.http.get<Object[]>(
      this.basic_url + '/ValidProductUser/' + email
    );
  }
  deleteProduct(id: any) {
    this.http
      .delete(`${this.basic_url}` + '/deletUserProduct/' + id, {
        responseType: 'text',
      })
      .subscribe(
        (res) => {
  ...
  sendMailToClient(username: any) {
    this.http
      .post(`${this.basic_url}` + '/forgetPassword/' + username, [], {
        responseType: 'text',
      })
      .subscribe(
        (res) => {
          ...
          this.route.navigateByUrl(
            '/auth/fogetPassword/verifyCode/' + username
          );
        },
        (error) => {
          const toast = this.toast.error({
  ...
  newCode(username: any, password: any) {
    ...
    this.http
      .post(
        `${this.basic_url}` + '/forgetPassword/newPassword/' + username,
        data,
        { responseType: 'text' }
      )
      .subscribe(
